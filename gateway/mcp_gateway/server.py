...
INSTRUCTIONS = (
    "Flutter Engineering MCP: one endpoint aggregating this toolkit's twelve Flutter/mobile "
    "servers -- code intelligence, architecture review, dependency management, test coverage, "
    "UI testing, crash analysis, Crashlytics, security scanning, API contract checking, "
    "performance analysis, CI/CD, and code migration. Each backend's tools are exposed under a "
    "namespaced name, '<backend_short_name>__<tool_name>' (e.g. 'flutterintel__index_project') "
    "-- call list_tools to see which backends actually connected and what each one offers; a "
    "backend that failed to start is simply absent, not an error."
)
...
async def main() -> None:
    ...
    async with AsyncExitStack() as stack:
        ...
        print(
            f"flutter-engineering-mcp: connected to {len(connected)} backend(s): "
            f"{', '.join(connected) or '(none)'}",
            file=sys.stderr,
        )
        if failed:
            print(
                f"flutter-engineering-mcp: skipped unreachable backend(s): {', '.join(failed)}",
                file=sys.stderr,
            )
